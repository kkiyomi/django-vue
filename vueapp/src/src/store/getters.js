...
export const currentSerie = (state) => {
  return state.serie_set.find(value => value && state.tvshow && value.tvshow.id === state.tvshow.id)
}

export const getNumberfromId = (state, getters) => {
  return [state.releases.find(value => value && getters.currentSerie && value.id === getters.currentSerie.current_release_id)]
}
